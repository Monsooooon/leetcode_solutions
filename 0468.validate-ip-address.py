...
class Solution:
    ...
    def validIPAddress1(self, IP):
        ...
        if IP.count(".") == 3 and all(isIPv4Num(i) for i in IP.split(".")): 
            return "IPv4"
        if IP.count(":") == 7 and all(isIPv6Num(i) for i in IP.split(":")): 
            return "IPv6"
        return "Neither"

    def validIPAddress(self, IP: str) -> str:
        if '.' in IP and Solution().isValidIPv4Address(IP):
            return "IPv4"
        elif ':' in IP and Solution().isValidIPv6Address(IP):
            return "IPv6"
        else:
            return "Neither"

    def isValidIPv4Address(self, IP: str) -> bool:
        nums = IP.split('.')
        if len(nums) != 4:
            return False
        for num in nums:
            if len(num) == 0 \
                or (len(num) > 1 and num[0] == '0') \
                or not num.isdigit() \
                or int(num) > 255:
                return False
        return True

    def isValidIPv6Address(self, IP: str) -> bool:
        import string
        nums = IP.split(':')
        if len(nums) != 8:
            return False
        for num in nums:
            if len(num) == 0 or len(num) > 4 \
                or not all(d in string.hexdigits for d in num):
                return False
        return True
